...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    vhat = predicted.T  #S*1
    z = outputVectors.dot(vhat)
    predicts = softmax(z)
    y = np.zeros_like(predicts)
    y[target] += 1
    cost = -np.log(predicts[target])

    gradZ = predicts-y #V*1
    gradPred = outputVectors.T.dot(gradZ)
    grad = gradZ.reshape((-1,1)).dot(vhat.reshape((1,-1)))
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    predicted = inputVectors[currentId]
    for word in contextWords:

        target = tokens[word]

        cost0,gradPred,grad = word2vecCostAndGradient(predicted,target,outputVectors,dataset)
        cost += cost0
