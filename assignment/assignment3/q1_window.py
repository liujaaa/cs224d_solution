...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        embedding_size = len(sentence[0] )
        sentence = start * window_size + list(np.array(sentence).reshape((-1,))) + end * window_size
        for i in range(len(labels)):
            windowed_data.append((sentence[i*embedding_size:(i+2*window_size+1)*embedding_size],labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(name='input',dtype=tf.int32,shape=(None,self.config.n_window_features))
        self.labels_placeholder = tf.placeholder(name='label',dtype=tf.int32,shape=(None,))
