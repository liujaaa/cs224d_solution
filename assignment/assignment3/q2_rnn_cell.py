...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable(name='W_x',dtype=tf.float32,shape=(self.input_size,self.output_size),initializer=tf.contrib.layers.xavier_initializer())
            W_h = tf.get_variable(name='W_h',dtype=tf.float32,shape=(self.output_size,self.output_size),initializer=tf.contrib.layers.xavier_initializer())
            b = tf.get_variable(name='b',dtype=tf.float32,shape=(self.output_size),initializer=tf.contrib.layers.xavier_initializer())
